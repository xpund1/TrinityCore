...
{
    for (Transport::PlayerSet::const_iterator itr = t->m_passengers.begin(); itr != t->m_passengers.end();)
    {
        Player* plr = *itr;
        ++itr;
    ...
    if (players.isEmpty())
        return;
                ...
                if(TeamInInstance == ALLIANCE)
                {
                    ...
                    {
						th->AddNPCPassengerInInstance(NPC_GB_ORGRIMS_HAMMER, 1.845810f, 1.268872f, 34.526218f, 1.5890f); //(1.845810f, 1.268872f, 34.526218f, 1.5890f)
    ...
    public:
        ...
        bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == 1)
            {
                ...
                if (curItemCount >= 1)
                {
                    pCreature->MonsterWhisper("You already have my jetpack!", player);
                    return false;
                }
                ...
                {
                    pCreature->MonsterWhisper("You do not have empty space for my jet-pack!", player);
                    return false;
                }
            }
